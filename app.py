...
if not st.session_state.logged_in:
    menu_auth = st.sidebar.selectbox("Menu", ["Login", "Create Account"])
    if menu_auth == "Create Account":
    ...
    if menu == "Transactions":
        ...
        with tab1:
            ...
            if type_ == "Transfer":
                ...
                from_acc = col_a.selectbox("From Account", list(acc_dict.keys()), key="from_account")
                to_acc = col_b.selectbox("To Account", [acc for acc in acc_dict.keys() if acc != from_acc], key="to_account")
            else:
    ...
    # ================= DASHBOARD =================
        ...
        if idx < len(months) - 1:
            p_inc, p_exp, _ = get_summary_clean(user_id, months[idx+1])
