...
def add_transaction(user_id, date, type_, category, subcategory, account_id, amount, notes):
    ...
    # Transfers should still move balance between accounts
...
if not st.session_state.logged_in:
        ...
        with tab1:
            ...
            if st.button("Save"):
                if type_ == "Transfer":
                    ...
                    # Transfer Out uses positive amount as signed_amount per your instruction
                    add_transaction(user_id, date, "Expense", "Transfer", "Transfer Out", from_id, amount, f"Transfer to {to_account} | {notes}")
                    # Transfer In uses positive amount as signed_amount per your instruction
    ...
    # ================= DASHBOARD =================
        ...
        # Net metrics excluding internal Transfers
        ...
        if not sub_df.empty:
            sel_cat = st.selectbox("Select Category for Detailed Analysis", sorted(sub_df["category"].unique()))
            sub_summary = sub_df[sub_df["category"] == sel_cat].groupby("subcategory")["amount"].sum().reset_index()
            st.plotly_chart(px.pie(sub_summary, names="subcategory", values="amount", title=f"Subcategory Breakdown for {sel_cat}", hole=0.4).update_traces(textinfo="percent+label"), use_container_width=True)
