...
def load_default_categories(user_id):
    category_data = [
        ("Salary","Base Salary","Income"), ("Salary","Bonus","Income"), ("Salary","Incentives","Income"),
        ("Business","Freelance","Income"), ("Business","Consulting","Income"), ("Business","Side Hustle","Income"),
        ("Returns","Interest Income","Income"), ("Returns","Dividends","Income"), ("Returns","Capital Gains","Income"),
        ("Returns","Rental Income","Income"), ("Returns","Business Profit Withdrawal","Income"),
        ("Refund","Tax Refund","Income"), ("Refund","Purchase Refund","Income"),
        ("Other Income","Gifts Received","Income"), ("Other Income","Misc Income","Income"), ("Other Income","Reimbursements","Income"),
        ("Food","Groceries","Expense"), ("Food","Dining Out","Expense"), ("Food","Snacks","Expense"), ("Food","Coffee","Expense"), ("Food","Food Delivery","Expense"),
        ("Housing","Rent","Expense"), ("Housing","Maintenance","Expense"), ("Housing","Repairs","Expense"), ("Housing","Furniture","Expense"), ("Housing","Appliances","Expense"),
        ("Utilities","Electricity","Expense"), ("Utilities","Water","Expense"), ("Utilities","Internet","Expense"), ("Utilities","Mobile Recharge","Expense"), ("Utilities","Gas","Expense"), ("Utilities","DTH / Cable","Expense"), ("Utilities","Society Charges","Expense"),
        ("Transport","Fuel","Expense"), ("Transport","Cab","Expense"), ("Transport","Metro","Expense"), ("Transport","Bus","Expense"), ("Transport","Parking","Expense"), ("Transport","Vehicle Maintenance","Expense"), ("Transport","Toll","Expense"), ("Transport","Insurance Renewal","Expense"),
        ("Shopping","Clothes","Expense"), ("Shopping","Shoes","Expense"), ("Shopping","Gadgets","Expense"), ("Shopping","Accessories","Expense"), ("Shopping","Household Items","Expense"),
        ("Entertainment","Movies","Expense"), ("Entertainment","OTT Subscriptions","Expense"), ("Entertainment","Events","Expense"), ("Entertainment","Games","Expense"), ("Entertainment","Music","Expense"),
        ("Travel","Flights","Expense"), ("Travel","Hotels","Expense"), ("Travel","Local Travel","Expense"), ("Travel","Travel Food","Expense"), ("Travel","Travel Shopping","Expense"),
        ("Health","Doctor Visit","Expense"), ("Health","Medicines","Expense"), ("Health","Tests","Expense"), ("Health","Gym","Expense"), ("Health","Supplements","Expense"), ("Health","Therapy / Counseling","Expense"),
        ("Family","Gifts","Expense"), ("Family","Support","Expense"), ("Family","Celebrations","Expense"), ("Family","Functions","Expense"),
        ("Education","Courses","Expense"), ("Education","Books","Expense"), ("Education","Certifications","Expense"), ("Education","Workshops","Expense"),
    ...
    for category, subcategory, type_ in category_data:
        cursor.execute("SELECT 1 FROM categories WHERE user_id=%s AND category=%s AND subcategory=%s", (user_id, category, subcategory))
        if not cursor.fetchone():
            cursor.execute("INSERT INTO categories (user_id, category, subcategory, type) VALUES (%s, %s, %s, %s)", (user_id, category, subcategory, type_))
    conn.commit()

def get_categories(user_id, type_):
    cursor.execute("SELECT DISTINCT category FROM categories WHERE user_id=%s AND type=%s", (user_id, type_))
    return [x[0] for x in cursor.fetchall()]

def get_subcategories(user_id, category):
    cursor.execute("SELECT subcategory FROM categories WHERE user_id=%s AND category=%s", (user_id, category))
    return [x[0] for x in cursor.fetchall()]

# ================= ACCOUNT FUNCTIONS =================
def get_accounts(user_id):
    cursor.execute("SELECT id, account_name, opening_balance FROM accounts WHERE user_id=%s", (user_id,))
    return cursor.fetchall()

def add_account(user_id, name, type_, balance, include_networth):
    cursor.execute("INSERT INTO accounts (user_id, account_name, account_type, opening_balance, include_in_networth) VALUES (%s, %s, %s, %s, %s)", (user_id, name, type_, balance, include_networth))
    conn.commit()
...
def add_transaction(user_id, date, type_, category, subcategory, account_id, amount, notes):
    ...
    cursor.execute("INSERT INTO transactions (user_id,date,type,category,subcategory,account,amount,signed_amount,tag,notes,created_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (user_id,date,type_,category,subcategory,account_id,amount,signed_amount,"",notes,datetime.now()))
...
def delete_transaction(transaction_id):
    cursor.execute("SELECT account, signed_amount FROM transactions WHERE id=%s", (transaction_id,))
...
if not st.session_state.logged_in:
    ...
    menu = st.radio("", ["Dashboard", "Transactions", "Accounts", "Categories", "Budgets"], horizontal=True)
    st.divider()
    ...
    # ================= CATEGORIES HUB =================
        ...
        parent_category = st.selectbox("Or Select Existing", ["None"] + existing_categories) if existing_categories else "None"
        subcat_name = st.text_input("Subcategory Name")
        if st.button("Save Category"):
            final_cat = category_name if parent_category == "None" else parent_category
            cursor.execute("SELECT 1 FROM categories WHERE user_id=%s AND category=%s AND subcategory=%s", (user_id, final_cat, subcat_name))
            if cursor.fetchone(): st.error("Exists")
            else:
                cursor.execute("INSERT INTO categories (user_id, category, subcategory, type) VALUES (%s, %s, %s, %s)", (user_id, final_cat, subcat_name, type_cat))
    ...
    # ================= BUDGETS HUB =================
        ...
        with tab1:
            ...
            if st.button("Save Budget"):
                set_budget(user_id, cat_b, amt_b); st.success("Saved")
        with tab2:
    ...
    # ================= DASHBOARD =================
        ...
        if not months: st.info("No data yet."); st.stop()
        selected_month = st.selectbox("Select Month", months)

        # Metrics strictly excluding internal Transfers
        ...
        # Metric Row 1: Net Flow
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Income (Net)", f"₹{inc:,.0f}")
        c2.metric("Expenses (Net)", f"₹{exp:,.0f}", delta_color="inverse")
        c3.metric("Investments", f"₹{inv:,.0f}")
        c4.metric("Savings", f"₹{sav:,.0f}")

        st.divider()

        # Metric Row 2: Burn and Assets (RESTORED)
        c5, c6, c7 = st.columns(3)
        today = datetime.now()
        day_count = today.day if selected_month == today.strftime('%Y-%m') else 30
        daily_burn = exp / day_count
        c5.metric("Daily Burn Rate", f"₹{daily_burn:,.0f}", help="Avg daily spend (excl. transfers).")
        ...
        # Visualization
        df_clean = pd.read_sql_query("SELECT * FROM transactions WHERE user_id=%s AND TO_CHAR(date, 'YYYY-MM')=%s AND category != 'Transfer'", conn, params=(user_id, selected_month))
        col_l, col_r = st.columns(2)
        with col_l:
            st.subheader("Composition")
            comp_df = pd.DataFrame({"Type": ["Income", "Expense", "Investment"], "Amount": [inc, exp, inv]})
            st.plotly_chart(px.bar(comp_df, x="Type", y="Amount", color_discrete_sequence=[main_color], text_auto=',.0f').update_layout(template="plotly_dark"), use_container_width=True)
        with col_r:
        ...
        if not sub_df.empty:
            sel_cat = st.selectbox("Select Category", sorted(sub_df["category"].unique()))
            sub_summary = sub_df[sub_df["category"] == sel_cat].groupby("subcategory")["amount"].sum().reset_index()
            st.plotly_chart(px.pie(sub_summary, names="subcategory", values="amount", hole=0.4).update_traces(textinfo="percent+label"), use_container_width=True)
        ...
        if not b_df.empty:
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(x=b_df["category"], y=b_df["spent"], name="Actual Spent", marker_color=[warning_color if x >= 0.9 else main_color for x in b_df["usage"]]))
            fig.add_trace(go.Scatter(x=b_df["category"], y=b_df["monthly_budget"], name="Limit", mode='lines+markers'))
        ...
        matrix_df = pd.read_sql_query("""
            ...
            FROM transactions t JOIN accounts a ON t.account = a.id
            WHERE t.user_id = %s AND TO_CHAR(t.date, 'YYYY-MM') = %s AND t.type = 'Expense' AND t.category != 'Transfer'
        ...
        if not matrix_df.empty:
            ...
            if grand_total > 0:
                pivot_matrix.loc["% Contribution"] = (account_totals / grand_total * 100).round(2)
